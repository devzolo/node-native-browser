...
inline int utf8_mbtowc(wchar_t* pwc, const unsigned char* src, int src_len)
{
    if (!pwc)
        return 0;
...
inline int utf8_wctomb(unsigned char* dest, wchar_t wc, int dest_size)
{
    if (!dest)
        return 0;
...
inline std::wstring utf8_mbstowcs_orig(const std::string& str)
{
    std::wstring wstr;
    wchar_t      wc;
    unsigned int sn = 0;
...
inline std::string utf8_wcstombs_orig(const std::wstring& wstr)
{
    std::string str;
    char        utf8[6];
    int         un = 0;
...
inline std::wstring utf8_mbstowcs(const std::string& str)
{
    const unsigned char* s = (const unsigned char*)str.c_str();
...
// Optimized
inline std::string utf8_wcstombs(const std::wstring& wstr)
{
...
inline std::string UTF16ToMbUTF8(const std::wstring& input)
{
    return utf8_wcstombs(input);
}

inline std::string UTF16ToMbUTF8(const wchar_t* input)
{
    if (input == nullptr)
        return "";
    return utf8_wcstombs(input);
