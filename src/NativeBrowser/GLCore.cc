...
GLuint GLCore::createShaderProgramFromCode(const char *vert, const char *frag)
{
  GLuint vertShader = compileShaderFromCode(GL_VERTEX_SHADER, vert);
  GLuint fragShader = compileShaderFromCode(GL_FRAGMENT_SHADER, frag);
  if (vertShader == 0 || fragShader == 0) {
    return 0;
  }
  return createShaderProgram(vertShader, fragShader);
